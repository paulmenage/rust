...
impl Futex for AtomicU32 {
            ...
            let r = unsafe {
                cfg_if::cfg_if! {
                    if #[cfg(target_os = "freebsd")] {
                        ...
                        libc::_umtx_op(
                            self as *const AtomicU32 as *mut _,
    ...
    fn wake(&self) -> bool {
        let ptr = self as *const AtomicU32;
    ...
    fn wait(&self, expected: u32, timeout: Option<Duration>) -> bool {
        ...
        let r = unsafe {
            libc::umtx_sleep(self as *const AtomicU32 as *const i32, expected as i32, timeout_ms)
        };
    ...
    #[cfg(target_os = "dragonfly")]
    fn wake(&self) -> bool {
        unsafe { libc::umtx_wakeup(self as *const AtomicU32 as *const i32, 1) };
        false
    }

    #[cfg(target_os = "dragonfly")]
    fn wake_all(&self) {
        unsafe { libc::umtx_wakeup(self as *const AtomicU32 as *const i32, i32::MAX) };
    }
    ...
    #[cfg(target_os = "emscripten")]
    fn wake(&self) -> bool {
        unsafe { emscripten_futex_wake(self, 1) > 0 }
    }

    #[cfg(target_os = "emscripten")]
    fn wake_all(&self) {
        unsafe { emscripten_futex_wake(self, i32::MAX) };
    }
    ...
    #[cfg(target_os = "fuchsia")]
    fn wake(&self) -> bool {
        unsafe { zircon::zx_futex_wake(self, 1) };
        false
    }

    #[cfg(target_os = "fuchsia")]
    fn wake_all(&self) {
        unsafe { zircon::zx_futex_wake(self, u32::MAX) };
    }
}
