...
impl Futex {
    ...
    pub fn wake(&self) -> bool {
        let ptr = &self.val as *const AtomicU32;
    ...
    pub fn wake(&self) -> bool {
        ...
        unsafe {
            libc::_umtx_op(
                &self.val as *const AtomicU32 as *mut _,
    ...
    pub fn wait(&self, expected: u32, timeout: Option<Duration>) -> bool {
        ...
        let r = unsafe {
            libc::futex(
                &self.val as *const AtomicU32 as *mut u32,
                libc::FUTEX_WAIT,
                expected as i32,
                timespec.as_ref().map_or(null(), |t| t as *const libc::timespec),
                null_mut(),
    ...
    pub fn wait(&self, expected: u32, timeout: Option<Duration>) -> bool {
        ...
        let r = unsafe {
            libc::umtx_sleep(
                &self.val as *const AtomicU32 as *const i32,
                expected as i32,
                timeout_ms,
            )
        };
    ...
    #[cfg(target_os = "dragonfly")]
    pub fn wake(&self) -> bool {
        unsafe { libc::umtx_wakeup(&self.val as *const AtomicU32 as *const i32, 1) };
        false
    }

    #[cfg(target_os = "dragonfly")]
    pub fn wake_all(&self) {
        unsafe { libc::umtx_wakeup(&self.val as *const AtomicU32 as *const i32, i32::MAX) };
    }
}
...
impl Futex {
    ...
    #[cfg(target_os = "emscripten")]
    pub fn wake(&self) -> bool {
        unsafe { emscripten_futex_wake(&self.val, 1) > 0 }
    }

    #[cfg(target_os = "emscripten")]
    pub fn wake_all(&self) {
        unsafe { emscripten_futex_wake(&self.val, i32::MAX) };
    }
}
...
impl Futex {
    ...
    #[cfg(target_os = "fuchsia")]
    pub fn wake(&self) -> bool {
        unsafe { zircon::zx_futex_wake(&self.val, 1) };
        false
    }

    #[cfg(target_os = "fuchsia")]
    pub fn wake_all(&self) {
        unsafe { zircon::zx_futex_wake(&self.val, u32::MAX) };
    }
}
